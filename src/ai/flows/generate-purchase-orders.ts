...
The quantity to order for an item is defined by its 'reorderQuantity'. If 'reorderQuantity' is not specified, you should recommend a sensible default (e.g., 2-4 weeks of stock, but for this task, let's default to ordering 50 units if not specified).

**Instructions:**

1.  **Analyze the inventory:** Iterate through the provided inventory data and identify all items that need to be reordered based on their quantity and reorder threshold. Ignore items that do not have a 'reorderThreshold' set.
...
3.  **Generate Purchase Orders:** For each supplier, create a separate purchase order proposal in markdown format. Each proposal should include:
    ...
    *   The current date.
    *   A markdown table with the columns: "Item Name", "Current Quantity", "Reorder Quantity".
